...
IF [%1]==[32] cl /O2 /Zi /c /Tptinker.c
IF [%1]==[32] link /OPT:REF /OPT:ICF /INCREMENTAL:NO /DEBUG /out:phantasm32.exe phantasm64.obj oracle.obj tinker.obj beasrc/BeaEngine.obj

IF [%1]==[bea] cd beasrc
IF [%1]==[bea] cl /O2 /Zi /c /I ../beainclude /Tp BeaEngine.c
IF [%1]==[bea] cd ..

IF [%1] EQU [] ECHO build {target}. Valid targets are "64", "32", "bea"
